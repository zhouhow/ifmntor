 ...
 */
@RestController
@RequestMapping("/pcap2/traindata")
public class TrainDataController {
    ...
    public Result findNodes(@PathVariable Long id) {
        return Result.oK(trainDataService.getById(id));
    }
    ...
    public Result findAll(PcapQueryVo vo) {
        ...
        if (!StringUtils.isEmpty(vo.getID())) {
            wrapper.eq(TrainData::getId, vo.getID());
        }
        return Result.oK(trainDataService.list(wrapper));
    }
}
